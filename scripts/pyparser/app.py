...
def prereq():
    ...
    if name in data:
        ...
        for index, req in enumerate(new_data['prerequisites']):
            reqName = req.split(': ')
            new_data['prerequisites'][index] = {
                reqName[0]: process(reqName[1])}
...
def recursion(obj, grade='CR'):
    ...
    if type(obj) == list and len(obj) >= 3:
        ...
        if comparator != 'with':
            obj = {'node': {'type': 'op', 'id': comparator, 'children': [
                recursion(req, grade=grade) for ind, req in enumerate(obj) if ind % 2 == 0]}}
        else:
            print(obj)
...
def process(st):
    ...
    logical = (pp.Keyword('AND') | pp.Keyword('OR') |
               pp.Keyword('WITH')).setName('logical')
