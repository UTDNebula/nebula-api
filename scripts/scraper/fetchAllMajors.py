            ...
            if matches and 'historical-studies' not in href:
                ...
                if major_info:
                    ...
                    json.dump(all_results, open(
                        'output/major_requirements.txt', 'w'))
...
def process_string(string):
    ...
    match_major_part = re.findall(
        r'Major(?:.*?)Courses: ([0-9]+)(?:-[0-9]+)* semester credit hours', string)
    if match_major_part:
...
def process_major_requirements():
    ...
    for school_key in data:
        ...
        for major_key in school:
            ...
            for type_key in major:
                new_school[major_key][process_types(
                    type_key)] = process_types(major[type_key])
    ...
    exceptions = {k: v for k, v in sorted(
        exceptions.items(), key=lambda item: item[1], reverse=True)}
