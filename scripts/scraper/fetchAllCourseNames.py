...
fp = urllib.request.urlopen(
    'https://catalog.utdallas.edu/2020/undergraduate/courses/')
...
for href in tbody.select('tr > td:first-child > a'):
    ...
    print('Getting class: ' + href.getText() +
          ' with ' + str(len(course_list)) + ' courses.')
    idx = 0
    for idx in range(0, min(len(course_list), len(course_name))):
        result += course_name[idx].getText() + '\n'
        count += 1
        idx += 1
    print('Finished getting ' + href.getText() +
          ', running total: ' + str(count))
